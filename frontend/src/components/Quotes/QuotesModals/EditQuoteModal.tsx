...
interface IEditQuoteModal {
  isOpen?: boolean;
  onClose: () => void;
  setQuotes: React.Dispatch<React.SetStateAction<TQuote | null>>;
  quoteToEdit: TQuote;
}
...
const EditQuoteModal = ({
  ...
  const [quote, setQuote] = useState<TUpdateQuote>({
    ...
    propertyId: quoteToEdit.propertyId,
    ...
    discountType: quoteToEdit.discountType,
    discountValue: quoteToEdit.discountValue,
  ...
  const handleUpdateQuote = async () => {
    ...
    const updatedLineItems: TUpdateLineItem[] = quote.lineItems.map((item) => {
      const updateItem: TUpdateLineItem = {
        quantity: item.quantity,
        name: item.name,
        unitPrice: item.unitPrice,
        description: item.description,
      ...
      if ((item as TUpdateLineItem).id && (item as TUpdateLineItem).id !== '') {
        updateItem.id = (item as TUpdateLineItem).id;
      }
      return updateItem;
    });

    const updatePayload: TUpdateQuote = {
      id: quoteToEdit.id,
    ...
    try {
      const response = await UpdateQuote(updatePayload);
      if (response.status === 200) {
        const { subtotal, discountAmount, taxAmount, total } =
          calculateQuoteTotals(response.data);
        const {
          assignedToUserId,
          assignedToUser,
          lineItems,
  ...
  const calculateQuoteTotals = (currentQuote: TUpdateQuote) => {
  ...
  useEffect(() => {
    if (quoteToEdit.discountValue > 0) {
      setIsAddDiscount(true);
    }
    if (quoteToEdit.taxRate > 0) {
      setIsAddTax(true);
    }
